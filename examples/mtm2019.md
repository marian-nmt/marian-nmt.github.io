...
- `--maxi-batch` preloads _M_ maxi-batches, i.e. _M_ x _N_ sentences.
- `--mini-batch-sort` sorts them according to source sentence length, this
...
multiple GPUs using the `--device` option.
...
try to improve the translation time, keeping BLEU at a good level.  Then grid
search the value of the length normalization parameter on <i>newstest2013</i>
and check if the improvement of BLEU transfers into the <i>newstest2014.de</i>.
</p>

<p>
What are the reasonable values for the beam size? What relative speed
improvement can be achieved by manipulating the maximum sentence length? What
is a danger of using too small value for the maximum length factor?
</p>
...
A reasonable values to consider for `--max-length-factor` are in a range of 1.2
to 3. The normalization parameter $$\alpha$$ can be grid searched for values in
a range of 0.2 to 2.
...
Start with the following command and set `--workspace`, `--device` and
...
<p>
How does a student model perform with the beam size of 1? What is the speed
improvement on CPU and GPU from a shortlist? Is there any improvement from the
auto-tunner?  What are architectures of student models and how large are they?
</p>
</div><!--//content-->
</div><!--//callout-block-->
